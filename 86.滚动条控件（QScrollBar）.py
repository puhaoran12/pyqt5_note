...
class ScrollBar(QWidget):
    ...
    def initUI(self):
        ...
        layout.addWidget(self.label)#添加到窗口

        self.scrollbar1=QScrollBar()
        self.scrollbar1.setMaximum(255)#如果不设置最小值则默认为0
        self.scrollbar1.sliderMoved.connect(self.sliderMoved)#设置信号

        self.scrollbar2 = QScrollBar()
        self.scrollbar2.setMaximum(255)  # 如果不设置最小值则默认为0
        self.scrollbar2.sliderMoved.connect(self.sliderMoved)

        self.scrollbar3 = QScrollBar()
        self.scrollbar3.setMaximum(255)  # 如果不设置最小值则默认为0
        self.scrollbar3.sliderMoved.connect(self.sliderMoved)

        self.scrollbar4 = QScrollBar()
        self.scrollbar4.setMaximum(255)  # 如果不设置最小值则默认为0
