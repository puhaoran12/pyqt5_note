...
class DataLocation(QWidget):
    ...
    def initUI(self):
        ...
        #搜索满足条件的cell
        text='(13,1)'
        items=tableWidget.findItems(text,QtCore.Qt.MatchExactly)#精确搜索
        #items = tableWidget.findItems(text, QtCore.Qt.MatchStartsWith)
        if len(items)>0:
            item=items[0]
            item.setBackground(QBrush(QColor(0,255,0)))#设置定位背景颜色
            item.setForeground(QBrush(QColor(255,0,0)))#设置定位字体颜色
